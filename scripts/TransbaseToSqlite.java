 ...
 * 
 * Usage: java -cp .:tbjdbc.jar:sqlite-jdbc.jar TransbaseToSqlite [output.db]
 */
public class TransbaseToSqlite {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            List<Integer> colJdbcTypes = new ArrayList<>();
            
            while (colsRs.next()) {
                colNames.add(colsRs.getString("COLUMN_NAME"));
                String typeName = colsRs.getString("TYPE_NAME").toUpperCase();
                colTypes.add(typeName);
                colJdbcTypes.add(colsRs.getInt("DATA_TYPE"));
            }
            colsRs.close();
