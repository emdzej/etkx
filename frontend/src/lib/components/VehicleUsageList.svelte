<script lang="ts">
  import type { PartUsageVehicle } from '$lib/api';

  let { usage = [] } = $props<{ usage?: PartUsageVehicle[] }>();
</script>

{#if usage.length === 0}
  <p class="text-sm text-slate-500 dark:text-slate-400">No vehicle usage data available.</p>
{:else}
